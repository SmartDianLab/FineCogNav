...
class EventHandler(object):
    ...
    def _open_scenes(self, ip: str , scen_ids: list):
        ...
        for index in range(len(scen_ids)):
            ...
            settings_path = str(ports[index])
            airsim_settings_write_content = json.dumps(airsim_settings)
            if not os.path.exists(str(CWD_DIR / 'airsim_plugin/settings' / settings_path)):
                os.makedirs(str(CWD_DIR / 'airsim_plugin/settings' / settings_path), exist_ok=True)
            with open(str(CWD_DIR / 'airsim_plugin/settings' / settings_path / 'settings.json'), 'w', encoding='utf-8') as dump_f:
            ...
            if choose_env_exe_paths[index] is None:
                ...
                if args.onscreen:
                    subprocess_execute = "bash {} -NoSound -NoVSync -GraphicsAdapter={} --settings {} ".format(
                        ...
                        gpus[index],
                        str(CWD_DIR / 'airsim_plugin/settings' / settings_path / 'settings.json'),
                    )
                    ...
                    print(f'offscreen scene: {subprocess_execute}')
