...
class AirVLNSimulatorClientTool:
    ...
    def getCollisionInfo(self):
        ...
        def _getCollisionInfo(airsim_client: airsim.VehicleClient):
            if airsim_client is None:
                raise Exception('error')
                return None
            return airsim_client.simGetCollisionInfo().has_collided
        ...
        for index_1 in range(len(self.airsim_clients)):
            ...
            for index_2 in range(len(self.airsim_clients[index_1])):
                threads[index_1].append(
                    MyThread(_getCollisionInfo, (self.airsim_clients[index_1][index_2],))
                )
        ...
        threads = []
        if not (np.array(thread_results) == True).all():
            logger.error('getCollisionInfo失败')
            return None
